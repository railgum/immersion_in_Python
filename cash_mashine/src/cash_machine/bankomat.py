...
class windows(tk.Tk):
    def __init__(self, cm, *args, **kwargs):
        ...
        self.wm_title('БАНК')
        self.iconphoto(False, tk.PhotoImage(file='icon_bank.png'))
        self.cm = cm
        ...
        for F in (MainPage, SidePage, CompletionScreen):
            frame = F(container, self, self.cm)
...
class MainPage(tk.Frame):
    def __init__(self, parent, controller, cm):
        tk.Frame.__init__(self, parent)
        self.cm = cm
        label = tk.Label(self, text='Добрый день, user!')
        label.pack(padx=10, pady=10)
        frame_balance = tk.Frame(self, borderwidth=3, width=100, height=100)
        btn_balance = tk.Button(frame_balance, text='Проверить баланс', command=self.show_balance)
        btn_balance.pack(anchor='n', pady=7, ipadx=38)

        balance_text = tk.Label(frame_balance)
        balance_text.pack()
        frame_balance.pack(padx=5, pady=5)

        frame_operation = tk.Frame(self, borderwidth=2, width=100, height=50)
        add_ = tk.Button(frame_operation, text='Внести',
                         command=lambda: controller.show_frame(SidePage, True))
        with_ = tk.Button(frame_operation, text='Снять',
                          command=lambda: controller.show_frame(SidePage, False))

        add_.grid(row=0, column=0, padx=10, pady=5)
        with_.grid(row=0, column=1, padx=10, pady=5)
        frame_operation.pack(padx=5, pady=5)
        close_btn = tk.Button(text='Закрыть и выйти', command=lambda: self.destroy())
    ...
    def show_balance(self):
        ...
        self.balance_text['text'] = self.cm.show()
